---
type: Finding
title: Properties near the Radnor Trail had higher reported values than those farther away
description: ULI reports a $69,139 proximity difference from a 2011 study but does not provide the model or uncertainty.
tags: [real estate, property values, trails, Pennsylvania]
sources:
  - id: uli2016
    resource: /sources/2016-active-transportation-real-estate.md
    title: "Active Transportation and Real Estate: The Next Frontier"
---

# Properties near the Radnor Trail had higher reported values than those farther away

## In brief

- ULI reports that properties within a quarter-mile (0.4 km) of Pennsylvania's Radnor Trail were valued $69,139 higher
  on average than area properties farther away, citing a 2011 study.[^uli2016]
- This is a proximity association, not a measured gain after trail construction; ULI provides no sample, model
  adjustments or uncertainty.

## Evidence and limits

The trail is in Radnor Township. ULI attributes the result to a **2011 GreenSpace Alliance and Delaware Valley
Regional Planning Commission study**. Its later Circuit profile rounds the same result to $69,000; that is a
repetition, not another study.[^uli2016]

The report does not reproduce the sample, property characteristics, valuation dates, model adjustments or uncertainty.
The result is a reported proximity association, not a measured $69,139 increase for each home following trail
construction or proof that trail access caused the difference. The underlying study was not independently reviewed.

## Related

- [Indianapolis parcel-value growth](indianapolis-cultural-trail-property-values.md): a different aggregate measure.
- [Active transportation and real estate](/topics/active-transportation-real-estate.md): why appreciation does not
  establish affordability benefits.

[^uli2016]: ULI (2016), p. 5, Radnor example; p. 41, Circuit Trails property-value discussion.
