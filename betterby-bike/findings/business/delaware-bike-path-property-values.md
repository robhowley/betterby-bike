---
type: Finding
title: CBC reports higher values near Delaware bike paths alongside positive-or-null review results
description: Secondary coverage reports at least $8,800 higher values within 50 metres of Delaware bike paths, without enough model detail to infer an installation effect.
tags: [business, real estate, property values, trails, Delaware]
sources:
  - id: cbc2019
    resource: /sources/2019-cbc-good-for-business.md
    title: Love them or hate them, research offers financial case for big city bike lanes
---

# CBC reports higher values near Delaware bike paths alongside positive-or-null review results

## In brief

- CBC reports that Delaware properties within 50 metres of bike paths were valued at least $8,800 higher than properties
  farther away, citing a 2006 paper.[^cbc2019]
- The paper's U.S. research review reportedly found mostly positive or no property-value effects, not uniformly positive
  results.[^cbc2019]
- The Delaware difference is an association, not appreciation caused by construction; CBC omits the valuation period,
  currency basis and uncertainty.

## Evidence and limits

CBC's 2019 article cites a **2006 University of Delaware paper** reviewing U.S. research. Most studies reportedly
found slight improvements in property values and ease of sale, or no effect. CBC reports a larger Delaware
property-value difference
when controlling for unspecified variables.[^cbc2019]

The [linked paper](http://128.175.63.72/projects/DOCUMENTS/bikepathfinal.pdf) was not separately reviewed. CBC does
not identify the valuation period, sample, precise adjusted estimate, currency basis or statistical uncertainty.
This is a reported near-versus-far association, not measured appreciation following construction or a protected
on-street lane premium. The review's null outcomes rule out treating its summary as uniformly positive.

Higher prices also do not establish improved housing affordability. CBC raises gentrification concerns but provides
no displacement estimate.[^cbc2019]

## Related

- [Active transportation and real estate](/topics/active-transportation-real-estate.md): compare proximity values,
  appreciation and affordability without pooling unlike measures.

[^cbc2019]: Dunne (2019), PDF p. 5, section 3, University of Delaware and affordability paragraphs.
